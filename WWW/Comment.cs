...
namespace Nicome.WWW.Comment
{
    ...
    class CommentClient
    {
        ...
        public async Task<List<ComApi.CommentBody.Json.JsonComment>> GetComment(WatchPage::BaseJson data, NicoContext context, bool kako = false)
        {
            ...
            var comments = await GetCommentData(data);
            crInfo.SetLastResNo(chandler.GetThreadInfo(comments).last_res);
            ...
            if (kako)
            {
                ...
                //初期whenデータを作成
                int _when = chandler.GetFirstComment(comments).date;

                List<ComApi.CommentBody.Json.JsonComment> kacomments;
                ComApi::CommentBody.Json.Chat firstComment;
                int i = 1;
                do
                {
                    logger.Log($"過去ログをダウンロード中...({i}件目)");
                    kacomments = await GetCommentData(data, _when);
                    try
                    {
                        firstComment = chandler.GetFirstComment(kacomments);
                    }
                    catch
                    {
                        break;
                    }
                    _when = firstComment.date - 1;
                    comments.AddRange(kacomments);
                    ++i;
                }
            ...
            comments.Sort((a, b) => { return a.chat == null || b.chat == null ? 0 : a.chat.no - b.chat.no; });
